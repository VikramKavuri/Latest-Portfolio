...
/**
 * BreakableCard — Kinetic shatter-box card that cracks and breaks apart
 * when shaken violently by dragging. Respawns after a cooldown.
 */
...
function useBreakableCard(onBreak) {
...
function DebrisChunk({ x, y, rot, path }) {
  ...
  return (
    ...
    >
      <path d={path} fill="#2C4A72" stroke="#B8860B" strokeWidth="1.5" />
    </svg>
  );
}

function CrackLines({ level }) {
...
export function BreakableCard({ title, description, skills = [], className, onBreak }) {
  const {
    isDragging,
    position,
    crackLevel,
  ...
  return (
    <div className={cn("relative w-full h-full", className)}>
      {/* Shadow Remnant */}
      <div className="absolute inset-0 bg-page-alt border-2 border-dashed border-navy/20 flex flex-col items-center justify-center z-0">
        <span className="font-display text-navy/30 text-xl uppercase tracking-widest mb-2">
          {title}
        </span>
        {isBroken && (
          <div className="w-24 h-3 border border-navy/20 bg-page relative overflow-hidden mt-2">
            <div
              className="absolute inset-0 bg-gold/60 transition-all duration-75 ease-linear"
      ...
      <div
        ...
        className={cn(
          "relative z-10 bg-page border-2 border-navy/30 p-5 cursor-grab active:cursor-grabbing select-none h-full flex flex-col justify-between overflow-hidden transition-[background-color]",
          !isDragging && !isBroken && "hover:animate-[hover-wiggle_0.8s_ease-in-out_infinite]",
          isBroken && "pointer-events-none",
          isFlashing && "bg-gold/10"
        )}
        style={{
          boxShadow: "4px 4px 0px #2C4A72",
          transform: isBroken
            ? fallTransform
            : `translate(${position.x}px, ${position.y}px) rotate(${position.x * 0.15}deg)`,
      ...
      >
        <div>
          ...
          <div className="flex justify-between items-start mb-3">
            <h3 className="font-display text-2xl text-navy uppercase tracking-tight leading-none">
              {title}
            </h3>
            <div className="w-6 h-6 bg-gold/80 rounded-full flex-shrink-0" />
          </div>

          {/* Description */}
          {description && (
            <p className="font-body text-sm text-navy/60 mb-3 leading-snug">{description}</p>
          )}
          ...
          {skills.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-2">
              {skills.map((skill) => (
                <div
                  key={skill.name}
                  className="group flex items-center gap-1.5 px-2 py-1 bg-navy/5 hover:bg-gold/10 transition-colors duration-200"
                  title={skill.name}
                >
                  <img
                    src={skill.icon}
                  ...
                  />
                  <span className="font-body text-[11px] text-navy/70 group-hover:text-navy transition-colors">
                    {skill.name}
                  </span>
                </div>
              ))}
        ...
        {/* Footer */}
        <div className="mt-4 border-t border-navy/15 pt-2 flex justify-between text-[10px] font-body uppercase tracking-widest text-navy/40">
        ...
        {crackLevel > 0.2 && (
          <div
            className="absolute inset-0 bg-gold mix-blend-multiply pointer-events-none"
