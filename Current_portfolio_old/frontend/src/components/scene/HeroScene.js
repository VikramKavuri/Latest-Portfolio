...
function HeroDataCore() {
  return (
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden" aria-hidden="true">
      <div
        className="absolute h-[34rem] w-[34rem] rounded-full opacity-70"
        style={{
          background:
...
export default function HeroScene({ onOpenBook, onWarmBook }) {
  ...
  // Content never waits for the remote 3D scene. Desktop loads it only after
  // the document is complete and the visitor has had time to see the hero.
  useEffect(() => {
    ...
    const scheduleSpline = () => {
      if (!isDesktop) return;
      splineTimer = window.setTimeout(() => setShowSpline(true), 5000);
    };
  ...
  return (
    ...
    >
      ...
      {/* Mobile atmosphere: no WebGL or remote scene on the critical path */}
